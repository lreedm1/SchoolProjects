...
// Full Name: Reed Lokken
// Campus ID: 908 375 4045
// WiscEmail: rlokken@wisc.edu
////////////////////////////////////////////////////////////////////////////////
...
public class RequestList {
  ...
  public static int addLast(String[] requests, int size, String newRequest) {
    if (size == requests.length) {
      System.out.println("Error! addLast is full and cannot add newRequest");
    } else {
  ...
  public static int getRequestsCountByTopic(String[] requests, int size, String topic)
    ...
    for (int i = 0; i < size; i++) {
      int splitLocation = requests[i].indexOf(":");
      if (requests[i].substring(splitLocation + 2).equals(topic)) {
        System.out.println("The topic " + topic + " is in the request " + requests[i]);
        count++;
      
      } else{
        System.out.println("The topic " + topic + " is not in the request " + requests[i]);
    }
  }
    return count; 
  }
  ...
  public static boolean testSuccessfulRemoveFirst() {
    ...
    String[] requests = new String[] {"Mouna: p1", "Jeff: eclipse", "Michelle: java17", "Reed: p1"};
    String[] requestsCopy = new String[] {"Mouna: p1", "Jeff: eclipse", "Michelle: java17", "Reed: p1"};
    int size = 4;
    // 2. Try to call RequestList.removeFirst() to remove the element at index 0 of requests
    int response = RequestList.removeFirst(requests, size);

    // 3. Check for bugs.
    if (response != size - 1) {
      System.out.println("Error! testSuccessfulRemoveFirst() did not return size-1");
      return false;
    }
    ...
    for (int i = 0; i < size - 1; i++) {
      if (!requests[i].equals(requestsCopy[i + 1])) {
        System.out.println("Error! Expected " + requestsCopy[i + 1] + " at index " + i + " but got "
            + requests[i]);
        return false;
      }
    }
