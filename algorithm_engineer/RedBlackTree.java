...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
    ...
    /**
     * Finds the lowest value in the tree and returns it
     * If the tree is empty, throws a NoSuchElementException
    ...
    /**
     * Finds the highest value in the tree and returns it
     * If the tree is empty, throws a NoSuchElementException
    ...
    private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
        // identify the relationship between the child and parent nodes
        if (child == null || parent == null || child.context[0] != parent) throw new IllegalArgumentException(
                "Neither the child nor parent argument can be null.");

        if (parent.context[2] == child) {
            // child is a right child of parent
            // perform a left rotation

            // set the child's parent to the grandparent of the parent and the grandparent's child to the child if the grandparent is not null
            if (parent.context[0] != null) {
                if (parent.context[0].context[1] == parent) {
            ...
            // if the child's left child is not null, set the parent's right child to the child's left child
            ...
            // child is a left child of parent
            ...
            // if the child's right child is not null, set the parent's left child to the child's right child
            ...
            throw new IllegalArgumentException("Child and parent nodes are not initially related that way.");
        }
    ...
    /**
     * Resolve any violations of the red-black tree
     * 1. Black height of all paths is not equal
     * 2. Red node has a red child
     * 3. Root is red
     * @return
     */
    protected void enforceRBTreePropertiesAfterInsert(Node<T> node) {
        // black parent case: if the parent is black or null,
        // set the black height of the node to 1 and return
        if (node == this.root || node.context[0].blackHeight == 1) {
            node.blackHeight = 1;
            return;
        }
        ...
        if (!node.context[0].isRightChild()) {
            ...
            if (node.context[0].context[0].context[2] == null || 
                ...
                this.enforceRBTreePropertiesAfterInsert(node.context[0].context[0]);
            }
        } else {
            // parent is a right child
            if (node.context[0].context[0].context[1] == null || 
